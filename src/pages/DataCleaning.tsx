    ...
    try {
      ...
      for (let i = 0; i < Math.min(20, rawData.length); i++) {
        ...
        if (currentOrderIdIdx !== -1) {
          ...
          const addressIndices = row
            .map((val, idx) => {
              const s = String(val).toLowerCase();
              return s === "address" ||
                s.includes("地址") ||
                s.includes("dirección")
                ? idx
                : -1;
            })
            .filter((idx) => idx !== -1);
          addressIdx =
            addressIndices.length > 0
              ? addressIndices[addressIndices.length - 1]
              : -1;
      ...
      if (headerRowIndex === -1 || orderIdIdx === -1) {
        throw new Error(
          '未找到包含 "Order #" (订单号) 的表头行。请确保上传了正确的 Mercado Libre 报表。',
        );
      }
      ...
      for (let i = headerRowIndex + 1; i < rawData.length; i++) {
        ...
        const statusStr =
          statusIdx !== -1 && row[statusIdx]
            ? String(row[statusIdx]).toLowerCase()
            : "";
        if (
          statusStr.includes("cancel") ||
          statusStr.includes("取消") ||
          statusStr.includes("cancela")
        ) {
          _type = "cancel";
        } else if (
          statusStr.includes("return") ||
          statusStr.includes("refund") ||
          statusStr.includes("退款") ||
          statusStr.includes("devolu")
        ) {
          _type = "refund";
        }
        ...
        let rawDate =
          orderDateIdx !== -1 && row[orderDateIdx]
            ? String(row[orderDateIdx])
            : "";
        ...
        parsedRecords.push({
          ...
          units:
            unitsIdx !== -1 && row[unitsIdx]
              ? parseInt(String(row[unitsIdx]))
              : 1,
          status: _type,
          buyer_address:
            addressIdx !== -1 && row[addressIdx] ? String(row[addressIdx]) : "",
        });
      }
      ...
      setSyncStatus({
        type: null,
        message: `解析成功，共 ${parsedRecords.length} 条数据，正在同步...`,
      });
      ...
      if (error) {
        if (
          ...
          throw new Error(
            "数据库 schema 需要更新！请在终端运行 cleaned_orders_migration_v2.sql 脚本以增加 sku 和 status 字段。",
          );
        }
        throw error;
      }
      ...
      setSyncStatus({
        type: "success",
        message: `成功同步 ${parsedRecords.length} 条数据！`,
      });
      ...
      <div className="v2-card mb-6">
        <div className="v2-card-header">
          <h2 className="v2-card-title">
            <span
              className={`w-2.5 h-2.5 rounded-full ${badgeColor} shadow-sm`}
            ></span>
            {title} ({orders.length})
          </h2>
        </div>
              ...
              {orders.map((order) => (
                <tr key={order.order_id} className="v2-table-tr group">
                  <td className="v2-table-td">
                    {order.status === "valid" && (
                      <span className="status-pill pill-success">有效</span>
                    )}
                    {order.status === "cancel" && (
                      <span className="status-pill pill-danger">取消</span>
                    )}
                    {order.status === "refund" && (
                      <span className="status-pill pill-warning">退款</span>
                    )}
                  </td>
                  <td className="v2-table-td font-mono font-bold text-sky-600">
                    {order.order_id}
                  </td>
                  <td className="v2-table-td text-slate-500 whitespace-nowrap">
                    {order.order_date.substring(0, 10)}
                  </td>
                  <td className="v2-table-td text-indigo-600 font-bold">
                    {order.sku}
                  </td>
                  <td className="v2-table-td text-center text-slate-900 font-bold">
                    {order.units}
                  </td>
                  <td
                    className="v2-table-td text-slate-500 leading-relaxed truncate max-w-[250px]"
      ...
      <div className="v2-inner-container">
        ...
        <header className="v2-header">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="v2-header-title">订单及销售数量清洗</h1>
              <p className="v2-header-subtitle">
                同步 Mercado Libre 原始数据，自动分类为有效、取消与退款
              </p>
            </div>
          </div>
        ...
        {!isLoading && data.length > 0 && (
          <div className="v2-stats-grid">
            <div className="v2-stat-card bg-white border-l-4 border-slate-400 shadow-sm">
              <span className="v2-stat-label text-slate-600">导入总订单</span>
              <div className="v2-stat-value text-slate-900">
            ...
            <div className="v2-stat-card bg-white/80 border-slate-200/60 shadow-lg relative overflow-hidden group">
              ...
              <div className="flex justify-between items-center">
                <div>
                  <span className="v2-stat-label text-slate-400 font-bold uppercase tracking-wider">
                    有效成交量
                  </span>
                  <div className="v2-stat-value text-emerald-600">
                    {validOrders.length.toLocaleString()}
                  </div>
                </div>
            ...
            <div className="v2-stat-card bg-white/80 border-slate-200/60 shadow-lg relative overflow-hidden group">
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <div className="v2-stat-value text-rose-600">
                    {cancelOrders.length.toLocaleString()}
                  </div>
                </div>
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <div className="v2-stat-value text-amber-600">
                    {refundOrders.length.toLocaleString()}
                  </div>
                </div>
          ...
          {isLoading ? (
            ...
            <div className="py-32 text-center text-slate-400 v2-card bg-white/50 border-dashed">
              ...
              <p className="text-base font-bold">
                暂无清洗记录，请上传 Mercado Libre 原始报表
              </p>
              <p className="text-xs opacity-60 mt-2">
                系统将自动识别订单状态并按 SKU 进行分类统计
              </p>
            </div>
          )}
        </div>
