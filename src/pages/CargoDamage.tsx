...
export default function CargoDamagePage() {
  ...
  const calculateTotalQuantity = () =>
    data.reduce((acc, curr) => acc + (curr.quantity || 0), 0);
  const calculateTotalValue = () =>
    data
      .reduce(
        (acc, curr) =>
          acc + Number(curr.quantity || 0) * Number(curr.skuValueCNY || 0),
        0,
      )
      .toFixed(2);
      ...
      <div className="v2-inner-container">
          ...
          <div className="v2-card bg-rose-500/5 border-rose-500/20 animate-in fade-in slide-in-from-top-4 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
              ...
              <div className="flex gap-2">
                <button
                  onClick={handleSave}
                  className="flex-1 bg-rose-600 hover:bg-rose-500 text-white rounded-lg h-9 text-xs font-bold transition-all active:scale-95"
                >
                  保存
                </button>
                <button
              ...
              <div className="flex flex-wrap gap-2">
                {REASONS.map((r) => (
                  <button
                    key={r}
                    onClick={() =>
                      setCurrentRecord({ ...currentRecord, reason: r })
                    }
        ...
        {!loading && data.length > 0 && (
          <div className="v2-stats-grid">
            ...
            <div className="v2-stat-card bg-white border-red-200/60">
              <div className="flex justify-between items-center">
                <div>
                  <span className="v2-stat-label text-red-600">
                    损失估值 (CNY)
                  </span>
                  <div className="v2-stat-value text-red-600">
                    ¥{calculateTotalValue()}
                  </div>
                </div>
                ...
                {loading ? (
                  ...
                  data.map((record) => (
                    <tr key={record.id} className="v2-table-tr group">
                      ...
                      <td className="v2-table-td text-slate-900 font-mono font-bold">
                        {record.quantity}
                      </td>
                      <td className="v2-table-td">
