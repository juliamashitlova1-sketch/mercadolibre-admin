...
export default function Overview() {
  const { skuData, allSkuData, operationLogs, fakeOrders, cargoDamage, onEditSku, onAddLog } = useOutletContext<ContextType>();
    ...
    <motion.div initial="hidden" animate="visible" variants={containerVariants} className="space-y-6">
        ...
        <div className="flex items-center gap-4 px-6 py-4">
          ...
          <div>
            ...
            <p className="text-slate-600 text-xs mt-0.5 max-w-2xl">
              ...
              净利润 <strong className={metrics.netProfitCNY >= 0 ? 'text-emerald-600' : 'text-rose-600'}>¥{metrics.netProfitCNY.toLocaleString(undefined, {minimumFractionDigits:1})} CNY</strong> (已扣除广告、测评或货损支出)。
            </p>
          </div>
        </div>
        <div className="px-6 flex items-center gap-6 border-l border-slate-200 h-full py-4 text-xs invisible md:visible">
          <div className="flex flex-col text-right">
            <span className="text-slate-500">期间额外支出</span>
            <span className="font-mono text-rose-500 font-bold">¥{metrics.totalExtraExpense.toFixed(1)}</span>
          </div>
          <div className="flex flex-col text-right">
                ...
                <div key={log.id || idx} className="p-3 mb-2 rounded-xl transition-all border border-transparent hover:border-slate-100 flex items-start gap-3 relative">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                       <span className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-wider">{log.date}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        <motion.div variants={itemVariants} className="glass-card rounded-2xl p-5 flex items-center justify-between">
          <span className="text-xs text-slate-600 font-medium">综合净利润率</span>
          <span className={`text-lg font-bold ${metrics.profitMargin >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{metrics.profitMargin.toFixed(1)}%</span>
        </motion.div>
        <motion.div variants={itemVariants} className="glass-card rounded-2xl p-5 flex items-center justify-between">
        ...
        <motion.div variants={itemVariants} className="glass-card rounded-2xl p-5 flex items-center justify-between">
          ...
          <span className="text-lg font-bold text-amber-600">{skuData.length > 0 ? `${Math.min(100, Math.floor((1 - skuData.filter(s => (s.stock || 0) < (s.avgSalesSinceListing || 1) * STOCK_HEALTH_THRESHOLD).length / skuData.length) * 100))}%` : 'N/A'}</span>
        </motion.div>
        <motion.div variants={itemVariants} className="glass-card rounded-2xl p-5 flex items-center justify-between">
          <span className="text-xs text-slate-600 font-medium">费用覆盖天数</span>
