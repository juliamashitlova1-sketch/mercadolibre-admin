...
export default function Operations() {
  const { operationLogs, onAddLog, refreshLogs } =
              ...
              <div className="v2-stat-value text-sky-600">
                {
                  operationLogs.filter((l) =>
                    l.date.startsWith(new Date().toISOString().substring(0, 7)),
                  ).length
                }
              </div>
            </div>
            <div className="v2-stat-card bg-white border-l-4 border-indigo-400 shadow-sm">
              <span className="v2-stat-label text-indigo-600 font-bold">
                广告优化次数
              </span>
              <div className="v2-stat-value text-indigo-600">
                {operationLogs.filter((l) => l.actionType === "Ads").length}
              </div>
            </div>
            <div className="v2-stat-card bg-white border-l-4 border-slate-400 shadow-sm">
                    ...
                    <tr key={log.id} className="v2-table-tr group">
                      ...
                      <td className="v2-table-td">
                        <div className="flex justify-center">
                          <div
                            className={`px-2.5 py-1 rounded-full border text-[10px] font-black tracking-wide shadow-sm flex items-center gap-1.5 ${getCategoryColor(log.actionType)}`}
                          >
