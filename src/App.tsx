      ...
      return (
        <div className="min-h-screen bg-red-50 flex items-center justify-center p-8">
          <div className="bg-white border-2 border-red-300 rounded-xl p-10 max-w-2xl text-center shadow-lg">
            ...
            <pre className="text-red-500 text-sm mb-6 p-4 bg-red-50 rounded-lg text-left overflow-auto">
              {this.state.error}
            </pre>
            <p className="text-gray-500 text-sm mb-4">
              请按 F12 打开控制台查看详细错误信息，并截图发给开发者
            </p>
            <button
...
export default function App() {
  const [auth, setAuth] = useState(
    sessionStorage.getItem("milyfly_auth") === "true",
  );
...
function AppContent() {
  ...
  const [skuEntryMode, setSkuEntryMode] = useState<"full" | "competitors">(
    "full",
  );
  ...
  return (
    <>
        ...
        >
          <Route element={<ContextWrapper contextValue={contextValue} />}>
            ...
            <Route
              ...
              element={<Navigate to="/fake-orders" replace />}
            />
            <Route
              path="/orders"
              element={<Navigate to="/fake-orders" replace />}
      ...
      <SKUEntry
        ...
        sku={selectedSku}
        mode={skuEntryMode}
        managedSkus={managedSkus}
        onSuccess={() => fetchSkuData()}
      />
      <ClaimEntry
        open={isClaimEntryOpen}
        onOpenChange={setIsClaimEntryOpen}
        claim={selectedClaim}
        onSuccess={() => console.log("Claim updated")}
      />
      <OperationEntry
        ...
        onSuccess={() => {
          fetchOperationLogs();
          console.log("Operation log saved");
        }}
      />
    </>
  );
