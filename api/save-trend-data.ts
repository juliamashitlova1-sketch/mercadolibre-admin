...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!sku || !crawl_date || !rows || !Array.isArray(rows) || rows.length === 0) {
    return res.status(400).json({ error: 'Missing required fields: sku, crawl_date, rows' });
  }
  ...
  try {
    ...
    const { error, count } = await supabase.from('sku_trend_data').insert(inserts);
